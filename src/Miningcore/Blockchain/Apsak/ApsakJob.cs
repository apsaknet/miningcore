...
public class ApsakXoShiRo256PlusPlus
{
    private ulong[] s = new ulong[4];

    public ApsakXoShiRo256PlusPlus(Span<byte> prePowHash)
    {
...
public class ApsakJob
{
    protected IMasterClock clock;
    public apsakd.RpcBlock BlockTemplate { get; private set; }
    ...
    public ApsakJob(IHashAlgorithm customBlockHeaderHasher, IHashAlgorithm customCoinbaseHasher, IHashAlgorithm customShareHasher)
    {
    ...
    protected virtual ushort[][] GenerateMatrix(Span<byte> prePowHash)
    {
        ...
        var generator = new ApsakXoShiRo256PlusPlus(prePowHash);
        while (true)
        {
            for (int i = 0; i < 64; i++)
            {
    ...
    protected virtual Span<byte> ComputeCoinbase(Span<byte> prePowHash, Span<byte> data)
    {
        ...
        for (int i = 0; i < 32; i++)
        {
            res[31-i] = (byte)(data[i] ^ ((byte)(product[2 * i] << 4) | (byte)product[2 * i + 1]));
        }
    ...
    protected virtual Span<byte> SerializeHeader(apsakd.RpcBlockHeader header, bool isPrePow = true, bool isLittleEndian = false)
    {
        ulong nonce = isPrePow ? 0 : header.Nonce;
        long timestamp = isPrePow ? 0 : header.Timestamp;
        Span<byte> hashBytes = stackalloc byte[32];
        //var blockHashBytes = Encoding.UTF8.GetBytes(ApsakConstants.CoinbaseBlockHash);
    ...
    protected virtual Share ProcessShareInternal(StratumConnection worker, string nonce)
    {
        ...
        var result = new Share
        {
            ...
            Difficulty = context.Difficulty / ApsakConstants.ShareMultiplier
        };
    ...
    public virtual Share ProcessShare(StratumConnection worker, string nonce)
    {
        ...
        if (nonce.Length <= (ApsakConstants.NonceLength - context.ExtraNonce1.Length))
        {
            nonce = context.ExtraNonce1.PadRight(ApsakConstants.NonceLength - context.ExtraNonce1.Length, '0') + nonce;
        }
    ...
    public void Init(apsakd.RpcBlock blockTemplate, string jobId)
    {
        Contract.RequiresNonNull(blockTemplate);
        Contract.RequiresNonNull(jobId);

        JobId = jobId;
        var target = new Target(ApsakUtils.CompactToBig(blockTemplate.Header.Bits));
        Difficulty = ApsakUtils.TargetToDifficulty(target.ToBigInteger()) * (double) ApsakConstants.MinHash / ApsakConstants.ShareMultiplier;
