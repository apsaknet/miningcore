...
using Miningcore.Blockchain.Apsak.Custom.Karlsencoin;
using Miningcore.Blockchain.Apsak.Custom.Pyrin;
using NLog;
...
public class ApsakJobManager : JobManagerBase<ApsakJob>
{
    public ApsakJobManager(
    ...
    private apsakd.ApsakdRPC.ApsakdRPCClient rpc;
    private apsakWalletd.ApsakWalletdRPC.ApsakWalletdRPCClient walletRpc;
    private string network;
    private readonly List<ApsakJob> validJobs = new();
    private readonly IExtraNonceProvider extraNonceProvider;
    private readonly IMasterClock clock;
    private ApsakPoolConfigExtra extraPoolConfig;
    private ApsakPaymentProcessingConfigExtra extraPoolPaymentProcessingConfig;
    ...
    protected IObservable<apsakd.RpcBlock> ApsakSubscribeNewBlockTemplate(CancellationToken ct, object payload = null,
        JsonSerializerSettings payloadJsonSerializerSettings = null)
    {
        return Observable.Defer(() => Observable.Create<apsakd.RpcBlock>(obs =>
        {
            ...
            Task.Run(async () =>
            {
                using(cts)
                {
                    retry_subscription:
                        ...
                        var requestNotifyNewBlockTemplate = new apsakd.ApsakdMessage();
                        requestNotifyNewBlockTemplate.NotifyNewBlockTemplateRequest = new apsakd.NotifyNewBlockTemplateRequestMessage();

                        // we need a request for retrieving BlockTemplate
                        var requestBlockTemplate = new apsakd.ApsakdMessage();
                        requestBlockTemplate.GetBlockTemplateRequest = new apsakd.GetBlockTemplateRequestMessage
                        {
                        ...
                        {
                            logger.Error(() => $"{ex.GetType().Name} '{ex.Message}' while subscribing to apsakd \"NewBlockTemplate\" notifications");
                        ...
                        while (!cts.IsCancellationRequested)
                        {
                            ...
                            retry_blocktemplate:
                                ...
                                {
                                    logger.Error(() => $"{ex.GetType().Name} '{ex.Message}' while streaming apsakd \"NewBlockTemplate\" notifications");
    ...
    private void SetupJobUpdates(CancellationToken ct)
    {
        ...
        var triggers = new List<IObservable<(string Via, apsakd.RpcBlock Data)>>
        {
            blockFound.Select(_ => (JobRefreshBy.BlockFound, (apsakd.RpcBlock) null))
        };

        // Listen to apsakd "NewBlockTemplate" notifications
        var getWorkApsakd = ApsakSubscribeNewBlockTemplate(ct)
            .Publish()
            .RefCount();

        triggers.Add(getWorkApsakd
        ...
        triggers.Add(Observable.Interval(TimeSpan.FromMilliseconds(1000))
            .Select(_ => (JobRefreshBy.Initial, (apsakd.RpcBlock) null))
    ...
    private ApsakJob CreateJob(long blockHeight)
    {
        switch(coin.Symbol)
        {
            case "CAS":
            case "HTN":
                ...
                {
                    string coinbaseBlockHash = ApsakConstants.CoinbaseBlockHash;
            ...
            case "KLS":
                ...
                if(customBlockHeaderHasher is not Blake2b)
                    customBlockHeaderHasher = new Blake2b(Encoding.UTF8.GetBytes(ApsakConstants.CoinbaseBlockHash));
                ...
                else if(karlsenNetwork == "testnet" && blockHeight >= KarlsencoinConstants.FishHashForkHeightTestnet)
                {
                    ...
                    if(customShareHasher is not CShake256)
                        customShareHasher = new CShake256(null, Encoding.UTF8.GetBytes(ApsakConstants.CoinbaseHeavyHash));
            ...
            case "PYI":
                ...
                else
                {
                    ...
                    if(customCoinbaseHasher is not CShake256)
                        customCoinbaseHasher = new CShake256(null, Encoding.UTF8.GetBytes(ApsakConstants.CoinbaseProofOfWorkHash));
        ...
        return new ApsakJob(customBlockHeaderHasher, customCoinbaseHasher, customShareHasher);
    }

    private async Task<bool> UpdateJob(CancellationToken ct, string via = null, apsakd.RpcBlock blockTemplate = null)
    {
        var cts = CancellationTokenSource.CreateLinkedTokenSource(ct);
    ...
    private async Task UpdateNetworkStatsAsync(CancellationToken ct)
    {
        ...
        {
            // update stats
            // we need a stream to communicate with Apsakd
            var stream = rpc.MessageStream(null, null, ct);

            var request = new apsakd.ApsakdMessage();
            request.EstimateNetworkHashesPerSecondRequest = new apsakd.EstimateNetworkHashesPerSecondRequestMessage
            {
                WindowSize = 1000,
            };
            ...
            request = new apsakd.ApsakdMessage();
            request.GetConnectedPeerInfoRequest = new apsakd.GetConnectedPeerInfoRequestMessage();
            await stream.RequestStream.WriteAsync(request);
    ...
    private async Task ShowDaemonSyncProgressAsync(CancellationToken ct)
    {
        ...
        var request = new apsakd.ApsakdMessage();
        request.GetInfoRequest = new apsakd.GetInfoRequestMessage();
    ...
    private async Task<bool> SubmitBlockAsync(CancellationToken ct, apsakd.RpcBlock block, object payload = null,
        JsonSerializerSettings payloadJsonSerializerSettings = null)
    {
        ...
        try
        {
            ...
            var request = new apsakd.ApsakdMessage();
            request.SubmitBlockRequest = new apsakd.SubmitBlockRequestMessage
            {
                Block = block,
    ...
    public ApsakCoinTemplate Coin => coin;

    public object[] GetSubscriberData(StratumConnection worker)
    {
        Contract.RequiresNonNull(worker);

        var context = worker.ContextAs<ApsakWorkerContext>();
        ...
        {
            context.ExtraNonce1,
            ApsakConstants.ExtranoncePlaceHolderLength - extraNonce1Size,
        };
    ...
    public bool ValidateIsBzMiner(string userAgent)
    {
        ...
        // Find matches
        MatchCollection matchesUserAgentBzMiner = ApsakConstants.RegexUserAgentBzMiner.Matches(userAgent);
    ...
    public bool ValidateIsGodMiner(string userAgent)
    {
        ...
        // Find matches
        MatchCollection matchesUserAgentGodMiner = ApsakConstants.RegexUserAgentGodMiner.Matches(userAgent);
    ...
    public bool ValidateIsIceRiverMiner(string userAgent)
    {
        ...
        // Find matches
        MatchCollection matchesUserAgentIceRiverMiner = ApsakConstants.RegexUserAgentIceRiverMiner.Matches(userAgent);
    ...
    protected override async Task PostStartInitAsync(CancellationToken ct)
    {
        ...
        var request = new apsakd.ApsakdMessage();
        request.GetCurrentNetworkRequest = new apsakd.GetCurrentNetworkRequestMessage();
        await Guard(() => stream.RequestStream.WriteAsync(request),
        ...
        var (apsakAddressUtility, errorApsakAddressUtility) = ApsakUtils.ValidateAddress(poolConfig.Address, network, coin.Symbol);
        if(errorApsakAddressUtility != null)
            throw new PoolStartupException($"Pool address: {poolConfig.Address} is invalid for network [{network}]: {errorApsakAddressUtility}", poolConfig.Id);
        else
            logger.Info(() => $"Pool address: {poolConfig.Address} => {ApsakConstants.ApsakAddressType[apsakAddressUtility.ApsakAddress.Version()]}");
        ...
        {
            // we need a call to communicate with apsakdWallet
            var call = walletRpc.ShowAddressesAsync(new apsakWalletd.ShowAddressesRequest(), null, null, ct);
    ...
    public override void Configure(PoolConfig pc, ClusterConfig cc)
    {
        ...
        extraPoolPaymentProcessingConfig = pc.PaymentProcessing.Extra.SafeExtensionDataAs<ApsakPaymentProcessingConfigExtra>();

        maxActiveJobs = extraPoolConfig?.MaxActiveJobs ?? 8;
        extraData = extraPoolConfig?.ExtraData ?? "SAKPool.beta";
        ...
        if(cc.PaymentProcessing?.Enabled == true && pc.PaymentProcessing?.Enabled == true)
        {
            ...
            walletDaemonEndpoints = pc.Daemons
                .Where(x => x.Category?.ToLower() == ApsakConstants.WalletDaemonCategory)
                .ToArray();

            if(walletDaemonEndpoints.Length == 0)
                throw new PoolStartupException("Wallet-RPC daemon is not configured (Daemon configuration for apsak-pools require an additional entry of category 'wallet' pointing to the wallet daemon)", pc.Id);
        }
    ...
    {
        logger.Debug(() => $"ProtobufDaemonRpcServiceName: {extraPoolConfig?.ProtobufDaemonRpcServiceName ?? ApsakConstants.ProtobufDaemonRpcServiceName}");

        rpc = ApsakClientFactory.CreateApsakdRPCClient(daemonEndpoints, extraPoolConfig?.ProtobufDaemonRpcServiceName ?? ApsakConstants.ProtobufDaemonRpcServiceName);

        // Payment-processing setup
        if(clusterConfig.PaymentProcessing?.Enabled == true && poolConfig.PaymentProcessing?.Enabled == true)
        {
            logger.Debug(() => $"ProtobufWalletRpcServiceName: {extraPoolConfig?.ProtobufWalletRpcServiceName ?? ApsakConstants.ProtobufWalletRpcServiceName}");

            walletRpc = ApsakClientFactory.CreateApsakWalletdRPCClient(walletDaemonEndpoints, extraPoolConfig?.ProtobufWalletRpcServiceName ?? ApsakConstants.ProtobufWalletRpcServiceName);
        }
    }
