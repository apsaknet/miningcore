...
public class ApsakPayoutHandler : PayoutHandlerBase,
    IPayoutHandler
{
    public ApsakPayoutHandler(
    ...
    protected apsakd.ApsakdRPC.ApsakdRPCClient rpc;
    protected apsakWalletd.ApsakWalletdRPC.ApsakWalletdRPCClient walletRpc;
    ...
    protected override string LogCategory => "Apsak Payout Handler";
    ...
    public virtual async Task ConfigureAsync(ClusterConfig cc, PoolConfig pc, CancellationToken ct)
    {
        ...
        logger = LogUtil.GetPoolScopedLogger(typeof(ApsakPayoutHandler), pc);
        ...
        if(walletDaemonEndpoints.Length == 0)
            throw new PaymentException("Wallet-RPC daemon is not configured (Daemon configuration for apsak-pools require an additional entry of category 'wallet' pointing to the wallet daemon)");
    ...
    public virtual async Task<Block[]> ClassifyBlocksAsync(IMiningPool pool, Block[] blocks, CancellationToken ct)
    {
        ...
        var coin = poolConfig.Template.As<ApsakCoinTemplate>();
        var pageSize = 100;
        ...
        for(var i = 0; i < pageCount; i++)
        {
            ...
            for(var j = 0; j < page.Length; j++)
            {
                ...
                var request = new apsakd.ApsakdMessage();
                request.GetBlockRequest = new apsakd.GetBlockRequestMessage
                {
                ...
                await foreach (var blockInfo in stream.ResponseStream.ReadAllAsync(ct))
                {
                    ...
                    else
                    {
                        ...
                        var requestConfirmations = new apsakd.ApsakdMessage();
                        requestConfirmations.GetBlocksRequest = new apsakd.GetBlocksRequestMessage
                        {
                        ...
                        if(block.ConfirmationProgress >= 1)
                        {
                            ...
                            foreach(var childrenHash in blockInfo.GetBlockResponse.Block.VerboseData.ChildrenHashes)
                            {
                                ...
                                var requestChildren = new apsakd.ApsakdMessage();
                                requestChildren.GetBlockRequest = new apsakd.GetBlockRequestMessage
                                {
                                    Hash = childrenHash,
                                    IncludeTransactions = true,
                                };
                                    ...
                                    if(childrenBlockRewardTransactions.Count > 0)
                                    {
                                        ...
                                        else if(mergeSetBluesHashes.Count > 0 && responseChildren.GetBlockResponse.Block.VerboseData.IsChainBlock)
                                        {
                                            ...
                                            if(childrenBlockRewardTransactions.First().Outputs[childrenPosition].VerboseData.ScriptPublicKeyAddress == poolConfig.Address)
                                            {
                                                ...
                                                logger.Debug(() => $"[{LogCategory}] Block {block.BlockHeight} - block child {responseChildren.GetBlockResponse.Block.Header.DaaScore} [{childrenHash}] provides {FormatAmount((decimal) (childrenBlockRewardTransactions.First().Outputs[childrenPosition].Amount / ApsakConstants.SmallestUnit))} => {coin.Symbol} address: {childrenBlockRewardTransactions.First().Outputs[childrenPosition].VerboseData.ScriptPublicKeyAddress} [{poolConfig.Address}]");
                                                blockReward += (decimal) (childrenBlockRewardTransactions.First().Outputs[childrenPosition].Amount / ApsakConstants.SmallestUnit);
                                            }
                                            else
                                                logger.Debug(() => $"[{LogCategory}] Block {block.BlockHeight} - block child {responseChildren.GetBlockResponse.Block.Header.DaaScore} [{childrenHash}] provides {FormatAmount(0.0m)}");
                                ...
                                if(blockRewardTransactions.Count > 0)
                                {
                                    ...
                                    if(amounts.Count > 0)
                                    {
                                        var totalAmount = amounts
                                            .Sum(x => (x.Amount / ApsakConstants.SmallestUnit));
    ...
    public virtual async Task PayoutAsync(IMiningPool pool, Balance[] balances, CancellationToken ct)
    {
        ...
        foreach(var pair in amounts)
        {
            ...
            var (apsakAddressUtility, errorApsakAddressUtility) = ApsakUtils.ValidateAddress(pair.Key, network, coin.Symbol);

            if(errorApsakAddressUtility != null)
                logger.Warn(()=> $"[{LogCategory}] Address {pair.Key} is not valid : {errorApsakAddressUtility}");
        }

        var callGetBalance = walletRpc.GetBalanceAsync(new apsakWalletd.GetBalanceRequest());
        var walletBalances = await Guard(() => callGetBalance.ResponseAsync,
            ex=> logger.Debug(ex));
        callGetBalance.Dispose();

        var walletBalancePending = (decimal) (walletBalances?.Pending == null ? 0 : walletBalances?.Pending) / ApsakConstants.SmallestUnit;
        var walletBalanceAvailable = (decimal) (walletBalances?.Available == null ? 0 : walletBalances?.Available) / ApsakConstants.SmallestUnit;
        ...
        await Parallel.ForEachAsync(amounts, parallelOptions, async (x, _ct) =>
        {
            ...
            await Guard(async () =>
            {
                ...
                var callSend = walletRpc.SendAsync(new apsakWalletd.SendRequest {
                    ToAddress = address.ToLower(),
                    Amount = (ulong) (amount * ApsakConstants.SmallestUnit),
                ...
                var sendTransaction = await Guard(() => callSend.ResponseAsync,
                    ex=> throw new PaymentException($"[{transferId}] apsakwalletd returned error: {ex}"));
                callSend.Dispose();
                ...
                if(string.IsNullOrEmpty(txId))
                    throw new Exception($"[{transferId}] apsakwalletd did not return a transaction id!");
                else
    ...
    {
        return difficulty * ApsakConstants.Pow2xDiff1TargetNumZero * (double) ApsakConstants.MinHash;
    }

    public double AdjustBlockEffort(double effort)
    {
        return effort * ApsakConstants.Pow2xDiff1TargetNumZero * (double) ApsakConstants.MinHash;
    }
